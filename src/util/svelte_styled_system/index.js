...
const themeGet = (theme, pathOrValue) => (pathOrValue ? dlv(theme, pathOrValue, pathOrValue) : pathOrValue);
      ...
      if (value.length > 1) {
        for (let i = 0; i < value.length; i++) {
          if (!mediaQueries[i]) {
            ...
            mediaQueries[i] = pseudoElementSelector ? `${pseudoElementSelector} { ` : '';
          }
